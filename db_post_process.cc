...
float PolygonScoreAcc(std::vector<cv::Point> contour, cv::Mat pred) {
  ...
  int xmin =
      clamp(int(floor(*(std::min_element(box_x.begin(), box_x.end())))), 0,
            width - 1);
  int xmax =
      clamp(int(ceil(*(std::max_element(box_x.begin(), box_x.end())))), 0,
            width - 1);
  int ymin =
      clamp(int(floor(*(std::min_element(box_y.begin(), box_y.end())))), 0,
            height - 1);
  int ymax =
      clamp(int(ceil(*(std::max_element(box_y.begin(), box_y.end())))), 0,
            height - 1);
